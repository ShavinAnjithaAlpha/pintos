...
int pintos_init(void)
{
  ...
  else
  {
    ...
    char cmd[32]; // declare the 32-bit character buffer to hold to the user commands
    ...
    while (strcmp(cmd, "exit") != 0)
    {
      ...
      {
        printf("exiting intercative shell...Bye!");
        break; // exit the interactive shell
      }
      ...
      {
        printf("Avaialable RAM: %'" PRIu32 "kB\n", init_ram_pages * PGSIZE / 1024);
      }
...
/* Read the line from the console into the cmd character buffer
   until carriage return or newline character is entered */
void read_line(char *cmd)
{
  ...
  while (chr != '\r' && chr != '\n')
  {
    ...
    cmd[idx++] = chr;
    putchar(chr);
    // handle the backspace key stroke
    if (chr == '\b')
    {
      cmd[--idx] = 0; // replace the \b with null character
      --idx;          // update the cursor index to the one character back
      putchar('\0');  // clear the last character by putting null
      putchar('\b');  // update the cursor position
    }
    else if (chr == '\n' || chr == '\r')
    {
      cmd[--idx] = '\0';
